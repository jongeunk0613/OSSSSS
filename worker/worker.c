...
child_proc(int conn)
{
	...
	char result[1024];
        ...
        child_pid=fork();
        if(child_pid==0){
		execl("/usr/bin/gcc","gcc","-o",compile_name,file_name,NULL);
        //        if(buildfailcheck = execl("/usr/bin/gcc","gcc","-o",compile_name,file_name,NULL)){
...
//		strcpy(result,"buildfail");
	...
	if(buildfailcheck != -1){//1
		...
		char out_file1[6];
                FILE* fptr1;
                char num1;
                char output1[1024],tmp1[1024];
		for(i=0;i<10;i++){
			strcpy(out_file1," .out");
			output1[0]='\0';
                	if(i!=9)num1 = '1'+i;
                	if(i!=9)out_file1[0]=num1;
                	else strcpy(out_file1,"10.out");
                	fptr1=fopen(out_file1,"r");
		
			while( fgets(tmp1,sizeof(tmp1),fptr1) != NULL){
                        	if(output1[0]=='\0'){
                               		 strcpy(output1,tmp1);
                       		}
                        	else strcat(output1,tmp1);
                        	if(output1[strlen(output1)-1]=='\n')output1[strlen(output1)-1]='\0';
                	}
			strcat(result,output1);
			strcat(result,"::");
		}
	}
        ...
        while (1) {
			...
			struct dirent *de; // Pointer for directory entry
		        ...
		        while((de = readdir(dr)) != NULL){
             	        if (strcmp(de->d_name, "worker.c") == 0 | strcmp(de->d_name, "worker") == 0 | strcmp(de->d_name, "Makefile") == 0)
                    	    continue;
               	        else
                        remove(de->d_name);
       		        }
